...
# build-linux.sh
#
# Builds a portable nmail Linux binary in a container and stages the
# install tree in dist/nmail-linux-<arch>-<libc>/. Requires docker.
#
# Usage:
...
# a host of each target architecture. The ccache persists under
# ~/.cache/nmail-dist/ (override with NMAIL_DIST_CACHE).
#
...
if [[ -n "${NMAIL_DIST_SKIP_IMAGE_BUILD:-}" && "${NMAIL_DIST_SKIP_IMAGE_BUILD}" != "0" ]]; then
  if ! docker image inspect "${IMAGE}" >/dev/null 2>&1; then
    echo "$0: NMAIL_DIST_SKIP_IMAGE_BUILD set but image ${IMAGE} is not loaded" >&2
    exit 1
  fi
...
mkdir -p "${CACHE_DIR}/ccache-${TARGET}"
