...
void Log::WriteCallstackToFd(int p_Fd, void* const* p_Callstack, int p_Size)
{
  ...
  // the musl binary is linked -no-pie) for offline `addr2line -e nmail.debug`.
  ...
  // walk in Util::GetCallstack is scoped to musl only, so no callstack is captured
  // here. Emit an explicit marker so the empty output is self-explanatory.
  UNUSED(p_Callstack);
  UNUSED(p_Size);
